...
odoo.define("web_widget_ckeditor.field_ckeditor", function (require) {
    ...
    // Replace core's html widget with ckeditor, depending on the ir.config_parameter
    // If that parameter is True, store the original html widget as `html_odoo` and
    // replace `html` with `ckeditor`.
    rpc.query({
        model: "ir.config_parameter",
        method: "get_param",
        args: ["web_widget_ckeditor.replace_html_widget"],
    }).then((value) => {
        if (value && ["1", "true"].includes(value.toLowerCase())) {
            field_registry.add("html_odoo", field_registry.get("html"));
