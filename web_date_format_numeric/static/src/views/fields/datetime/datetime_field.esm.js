...
// Falsy option values are not persisted in XML
// (e.g. {numeric: false} is dropped, while {numeric: "false"} is kept).
// Stringify both choices so exprToBoolean can resolve them correctly.
const numericSupportedOptions = _dateField.supportedOptions.map((option) =>
    option.name !== "numeric"
        ? option
        : {
              ...option,
...
const useDateFormatNumeric = (baseField) => {
    ...
    return {
        ...baseField,
        supportedOptions: numericSupportedOptions,
