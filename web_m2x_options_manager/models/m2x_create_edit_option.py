...
class M2xCreateEditOption(models.Model):
    ...
    option_create = fields.Selection(
        ...
        "* Remove: option set to False only if not already present\n"
        "* Force Remove: option is always set to False",
        required=True,
    ...
    def apply(self, node, model_name):
        ...
        record architecture).
        """
        opt = self.get(model_name, node.attrib["name"])
        if opt:
            ...
            opt._update_options(node_options)
            node.set("options", str(node_options))

    def _update_options(self, node_options):
        self.ensure_one()
        if self.option_create == "force_remove" or (
            self.option_create == "remove" and "create" not in node_options
        ):
            node_options["create"] = False
        if self.option_create_edit == "force_remove" or (
            self.option_create_edit == "remove" and "create_edit" not in node_options
        ):
            node_options["create_edit"] = False

    @api.model
    def get(self, model_name, field_name):
        """Returns specific option record for ``field_name`` in ``model_name``
